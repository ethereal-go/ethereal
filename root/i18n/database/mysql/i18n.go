...
*/
func (schema graphQLI18n) Merge(merge map[string]map[string]string) localeI18n.Locale {
...
func (schema graphQLI18n) Fill() {
	for locale, mapValues := range schema.structure {
		for key, value := range mapValues {
			schema.i18n.SaveTranslation(&i18n.Translation{Key: key, Locale: locale, Value: value})
		}
	}
}
